...
def setup():
    size(220,100)
    global library, Madzia, Kewin
    books = ["Naocznosc", "Sens Sztuki", "Harry Potter", "Europa Jesienia"]
    library = Library(books) # bo biblioteka bez książek, to nie biblioteka
    Kewin = Customer() # miał być odany, nie zastąpiony
...
def mouseClicked(): # poklikajcie kilkakrotnie w przyciski: wypożyczneie dwa razy tej samej książki, próba zwrócenia bez posiadania żadnej? Kto podejmuje działanie? 
    if mouseX >100 and mouseX<200:
        ...
        if mouseY >40 and mouseY <60:
            library.addBook(Madzia.returnBook()) # teraz przy próbie wypożyczeniazwróćenia kod nie działą, bo nie znajdzie Madzi
