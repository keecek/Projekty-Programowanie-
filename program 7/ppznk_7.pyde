...
class MouseDeer(Pet):
    def __init__(self, name):
        self.name = name
    def speak(self):
        text('myszojelen', random(50, width-20), random(60, height-40))
        return 'myszojeleń'
...
def setup():
    ...
    myszojelen = MouseDeer('myszojelen')
    #janusz = Bunny() # odkomentujcie i sprawdźcie błąd, usuńcie dziedziczenie po Pet z klasy i sprawdźcie ponownie
    global list_of_pets
    list_of_pets = [rex, benio, skrypcik, myszojelen] # lista zawiera różne typy zwierząt
